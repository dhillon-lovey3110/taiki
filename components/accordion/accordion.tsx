...
export default function Accordion(props: IAccordionProps) {
  ...
  return (
    <>
        <h2 className='cursor-pointer text-[24px] font-agradir font-ppagradirBold font-extrabold mb-10' onClick={ () => { setShow(!show); }}>{props.title} {show==false?
        ...
        <div className={show==false?'hidden':'text-[18px] font-ppagradir font-agradir mb-10 font-extralight text-dark'}>{props.content}</div>
        <hr className='mb-10' />
    </>
  );
}