...
export default function Accordion(props: IAccordionProps) {
  ...
  return (
    <>
        ...
        <div className={"transition-all duration-500  " + (show==false?'opacity-0 h-0':'opacity-100 h-28')}>
          <div className={"text-[18px] font-ppagradir font-agradir mb-10 font-extralight text-dark transition-all duration-1000 " + (show==false?"opacity-0":'opacity-100')}>{props.content}</div>
        </div>
        <hr className='mb-10' />
    </>
  );
